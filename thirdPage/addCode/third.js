    ...
    $(".gameUpper").click(function() {
        ...
        if (talkCount === 6 && checkAnnounce === 0) {
            ...
            setTimeout(function() {
                ...
                $(".questionText > p").html(
                    '魔法陣を解いて偶数 ( 2で割り切れる数字。 2,4,6,8.... ) の数字の少ない順によめ！ (間違った答えを入力すればヒントを見れるようになります)'
                )
            ...
            setTimeout(function() {
                ...
                $(".questionText > p").html(
                    '漢字の中に隠れているカタカナを見つけろ！ (※三文字目には濁点がつきます) (間違った答えを入力すればヒントを見れるようになります)'
                )
            ...
            setTimeout(function() {
                ...
                $(".questionText > p").html(
                    '下の暗号を解け！ (間違った答えを入力すればヒントを見れるようになります)'
                )
        ...
        setTimeout(function() {
            ...
            $(".questionText > h2").html(
                'Q. 謎といて、技を発動せよ！ (間違った答えを入力すればヒントを見れるようになります)'
            )
        ...
        setTimeout(function() {
            ...
            setTimeout(function() {
                ...
                $(".questionText > p").html(
                    '書き順通りによめ (※10文字目には伸ばし棒が、11文字目には濁点が入ります) (間違った答えを入力すればヒントを見れるようになります)'
                )
            ...
            $(".attackButton").click(function() {
                ...
                if (answerStorage === 'ニュークエクスプロージョン' || answerStorage === 'ニュークエクスプローション') {


                    $(".displayResult h2").html('A. "ニューク" エクスプロージョン (エクスプローション )')

                    $(".commentaryText p").html(
                        '問題に不備があったため、ニュークエクスプローションでも正解です！　出来次第修正します！原子力発電は”ウラン”というものを使って電気を生み出しています　原子力発電は地球に悪いみたいなイメージがあるかもしれないけど、実は環境に悪いってわけではないんだ！　確かにウランの量には限りがあるし、いつか使えなくなってしまうものではあるけど、実は一度使われたウランのほとんどをもう一度使うことができるか有効に活用することもできるんだ！　それに、電気を作るときに二酸化炭素(CO2)をほとんど排出しないから環境に優しいとも言われているんだ！　あと、ちなみに”ニュークエクスプローション”は、”ニューク”が核のという海があり、エクスプローション (エクスプロージョン)は爆発みたいな意味があるよ！'
                    )
